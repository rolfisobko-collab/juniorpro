...
export async function POST() {
  try {
    ...
    const refresh = jar.get("tz_refresh")?.value
    ...
    const newAccess = await signAccessToken({ sub: match.userId, typ: "user" }, "15m")
    ...
    jar.set("tz_access", newAccess, { ...cookieOptions(), maxAge: 60 * 15 })
    jar.set("tz_refresh", newRefresh, { ...cookieOptions(), maxAge: 60 * 60 * 24 * 30 })
